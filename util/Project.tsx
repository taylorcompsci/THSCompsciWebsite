...
{
    return (
        <Link href={project.projectLink} replace={false} target="_blank" rel="noopener noreferrer">
            <div className="group border border-white w-100 h-100 overflow-hidden">
                ...
                <div className="flex flex-col items-start p-4">
                    ...
                    <p className="text-xl text-gray-500">@{project.author?.replace(" ","_").toLowerCase()}</p>
                    <p className="text-[90%] text-wrap text-left">{project.description}</p>
                </div>
            </div>
        </Link>
    )
