    ...
    useEffect(()=>{
        ...
        if(triggerRef.current)
        {
            ...
            const observer = new IntersectionObserver(async ()=>{
                ...
                const result = (await fetchGalleryImages(prevToken).catch((e)=> {setErrorMessage(e); return undefined}));
    ...
    return (
        <Scroll className="flex flex-col justify-center items-center mt-10">
            ...
            <div className="flex-wrap flex justify-center gap-3">
                {useFiles?.map((val)=> <Image className="w-100 h-70" src={val} alt={`Picture from ${val}`} key={`Image-${val}`} width={300} height={400} placeholder="blur" blurDataURL="/assets/placeholder.webp"/>)}
